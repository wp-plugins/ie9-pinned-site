...
class ie9PinnedSiteUploadFiles {	
	var $entryType;
	var $order;
	
	function ie9PinnedSiteUploadFiles($data, $entryType, $order) {		
	...
	function render4Task(){
		if ( is_user_logged_in() && current_user_can( 'upload_files' )) {
			echo '<meta name="msapplication-task" content="name=' . __( 'Upload new media', 'IE9-Pinned-Site' ) . ';action-uri=' . admin_url() . 'media-new.php;icon-uri=' . plugins_url('images/media.ico', __FILE__) . '" />'."\r\n";
		}
	}
	...
	function render4AdminPage(){
        ...
        <li name="<?php echo $prefix ?>" id="<?php echo $prefix ?>">
            ...
            <span class="settings">
  			...
  			<input type="hidden" id="<?php echo $prefix ?>_type" name="<?php echo $prefix ?>_type" value="UploadFiles">
		</li>
        <?php
	}
	
	function GetSettingObject($entryType, $index){
		$uploadFilesEntry =  new stdClass();
		$uploadFilesEntry->type = 'UploadFiles';
		
		return $uploadFilesEntry;
	}
}
?>